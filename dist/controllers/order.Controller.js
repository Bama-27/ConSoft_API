...
exports.OrderController = {
    ...
    createReview: async (req, res) => {
        try {
            ...
            const order = await order_model_1.OrderModel.findById(orderId).populate('user', '_id');
            if (!order)
                return res.status(404).json({ message: 'Order not found' });
            ...
            const existing = order.reviews?.some((r) => String(r?.user) === String(userId));
            if (existing) {
            ...
            await order.save();
            const createdReview = order.reviews[order.reviews.length - 1];
    ...
    listReviews: async (req, res) => {
        try {
            const orderId = req.params.id;
            const order = await order_model_1.OrderModel.findById(orderId).select('user reviews').lean();
            if (!order)
                return res.status(404).json({ message: 'Order not found' });
            return res.json({ ok: true, reviews: order.reviews ?? [] });
        }
        catch (_e) {
    ...
    listAllReviews: async (_req, res) => {
        try {
            const orders = await order_model_1.OrderModel.find({ 'reviews.0': { $exists: true } })
                .select('reviews user')
                .populate('user', 'name')
                .lean();
            const allReviews = orders.flatMap((order) => (order.reviews || []).map((review) => ({
                ...review,
                orderId: order._id,
    ...
    create: async (req, res) => {
        try {
            ...
            const populatedOrder = await order_model_1.OrderModel.findById(order._id)
                .populate('user', 'name email')
                .populate('items.id_servicio')
                .populate('items.id_producto')
                .lean();
    ...
    createForMe: async (req, res) => {
        try {
            ...
            if (quotationId) {
                try {
                    const QuotationModel = mongoose_1.default.model('Cotizacion');
                    const quotation = await QuotationModel.findById(quotationId).lean();
                    if (quotation && quotation.initialPayment) {
                        initialPaymentAmount = quotation.initialPayment.amount || 0;
                        initialPaymentMethod = quotation.initialPayment.method;
                    }
                }
                catch (err) {
            ...
            const [products, services] = await Promise.all([
                productIds.size
                    ? product_model_1.ProductModel.find({ _id: { $in: Array.from(productIds) } }).select('_id imageUrl').lean()
                    : [],
                serviceIds.size
                    ? service_model_1.ServiceModel.find({ _id: { $in: Array.from(serviceIds) } }).select('_id imageUrl').lean()
                    : [],
            ]);
            ...
            const total = normalizedItems.reduce((sum, item) => sum + (item.valor || 0), 0);
            let status = 'Pendiente';
            let payments = [];
            let productionStartedAt = null;
